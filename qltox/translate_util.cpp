...
static int utf8Bytes(uint32_t cp) {
    if (cp < 0x80) { return 1; }
...
static CharClass classifyChar(uint32_t cp) {
...
bool isNeedTranslateToChinese(const QString& text) {
    ...
    for (int i = 0; i < text.length(); ++i) {
        ...
        CharClass cc = classifyChar(cp);
        int bytes = utf8Bytes(cp);
        if (cc == CC_LATIN) {
            need += bytes;
        } else if (cc == CC_JAPANESE || cc == CC_KOREAN) {
            need += bytes * 2;
        } else {
            noneed += bytes;
        }
