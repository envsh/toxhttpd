...
- **ChatElement** (`chatview.h`, formerly `ChatMessage`): per-message data + layout cache, phased evolution:
  **Phase 2** (done) — `ChatElement` caches `cachedWidth`/`height` per-element (TG-style). `relayout()` skips cached items. `appendMessage` is O(1). `triggerRelayout(int msgIndex)` invalidates only one element.
